...
class QBCVector extends ItemCore
{
	IsSingleLine() { return true; }
	Initialize() { this.values = [0.0, 0.0, 0.0]; }
	ReadSharedValue() { var floatsStart = this.reader.UInt32(); }
	GetItemInfoType() { return QBC.constants.TypeBindings['Vector']; }
	...
	{ 
		...
		this.values = [ this.reader.Float(), this.reader.Float(), this.reader.Float() ];
	}
	...
	Serialize()
	{
		if (this.InScript())
		{
			this.writer.UInt8(QBC.constants.ESCRIPTTOKEN_VECTOR);
