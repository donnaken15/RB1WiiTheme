...
//
//	KEY HANDLER
//	Simple class for QBKey operations
//
// ---------------------------------------------------
...
function GetQBKey(txt, reversed = false)
{
	...
	var finalBuffer = (reversed ? rev : hex);
	return reversed ? hex.readUInt32LE(0) : hex.readUInt32BE(0);
}
...
class KeyHandler
{
    ...
    ToKey(txt, reversed = false)
	{
		...
		if (typeof(txt) == "number")
			txt = "0x" + txt.toString(16).padStart(8, "0");
    ...
    //-----------------------
    // Pull from our core Key database.
    //-----------------------
    
    PullKeys(keyDir, depth=0)
    {
        // Not super-recursive, just a little bit
        if (depth > 2)
            return;
            
        QBC.Debug(("".padEnd(depth*2, " ")) + "Pulling keys from " + keyDir + "...");
        
        if (!fs.existsSync(keyDir))
            return;
            
        // Read files in the directory.
        // We will not read subfolders, iffy.
        
        var files = fs.readdirSync(keyDir);
        
        for (const file of files)
        {
            var fPath = path.join(keyDir, file);
            
            if (fs.lstatSync(fPath).isDirectory())
            {
                this.PullKeys(fPath, depth+1);
                continue;
            }
